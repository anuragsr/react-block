...
export default class TagGroupBlock extends Component {
  ...
  submit = () => {
    ...
    if(this.state.tags.length){
      ...
      let genTag = this.state.tags.filter(x => x.isMain)
      if(!genTag.length) {
        alert('Please choose a general tag!')
        return
      }
      else {
        genTag = genTag[0].id
        ...
        const request = {
          tags_ids: this.state.tags.filter(x => !x.isMain).map(x => x.id),
          ML_attraction: this.state.ml?this.state.att.auto:"None",
          editor_attraction: this.state.att.manual, 
          editor_id: 1,
          bot_id: this.state.currBot.id,
          general_tag_id: genTag,
        }
        l(request)
    
        this.http
        .post('/api/v1/send_attraction_for_tag_group', request, auth)
        .then(res => {
          // l(res.data)
          //  Show notif, undo
          this.getSuggTags()
      ...
      {this.state.showNotif && <div className="block-notif">
        {this.state.notifType === "submit" && <>
          <div className="col-lg-11 px-0">
            <div className="title">
              <span>New tag group block created!&nbsp;&nbsp;</span>
        ...
        <div className="col-lg-12 px-0 text-center">
          <div className="title">
            <span>Tag group block deleted successfully!&nbsp;&nbsp;</span>
              ...
              <div className="tag-outer">
                ...
                {this.state.showTags &&
                  <TagsComponent
                    ...
                    addTag={this.tagAdded}
                    isGroup={true}
                    showAuto={this.state.showAuto}
                    mainChosen={this.state.mainChosen}
                  />}
              </div>
            </div>          
          </div>
