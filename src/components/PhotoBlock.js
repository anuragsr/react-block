...
let randHex = () => {
  ...
  getCoords = (data, type, subtype) => {
    ...
    switch (type) {
      ...
      case "circle":
        switch (subtype) {
          case "percent":
            coords = data.object_coords.map(c =>
              parseFloat(parseFloat(c).toFixed(2))
            );
            returnVar.push(
              {
                x: coords[0] * width,
  ...
  sortPointsClockwise = points => {
    ...
    points.sort((a, b) => a.angle - b.angle).unshift(points.pop());
  },
  blkRef,
  imgInnerRef,
  imgRef,
...
export default class PhotoBlock extends Component {
  ...
  getPhotos = (id, type) => {
    ...
    // l(params)
    this.http.get("/api/v1/photos", lastParams, auth).then(res => {
  ...
  nextPhoto = afterSubmit => {
    ...
    if (!afterSubmit) {
      ...
      this.http.put("/api/v1/submit_photo", req).then(res => {
        l(res);
      });
      ...
      this.http.get("/api/v1/photos", params, auth).then(res => {
        let results = res.data.results;
        if (results.length) photos.push(results[0]);
  ...
  catChanged = e => {
    let currCat = this.state.categories.filter(cat => {
    ...
    let params,
      ...
      getPointAsSquare = (start, end) => {
        ...
        let dx = line.x1 - line.x0,
          dy = line.y1 - line.y0,
          t =
            ((end.x - line.x0) * dx + (end.y - line.y0) * dy) /
            (dx * dx + dy * dy);
    ...
    bg.on("pointerdown", e => {
      ...
      switch (this.state.adding) {
        case "polygon":
          ...
          tempCtn.controlPoints = this.drawControlPoints(
            params.points,
            "polygon"
          );
  ...
  clearTempCtn = () => {
    Object.keys(tempCtn).forEach(key => {
      if (key === "controlPoints")
        tempCtn[key].forEach(obj => this.ctn.removeChild(obj));
  ...
  addInteraction = (graphic, eventType) => {
    ...
    function getClosestPointOnLine(center, idx, pos) {
      ...
      let dx = line.x1 - line.x0,
        dy = line.y1 - line.y0,
        t =
          ((pos.x - line.x0) * dx + (pos.y - line.y0) * dy) /
          (dx * dx + dy * dy);
      ...
      switch (gr.type) {
        ...
        default:
          ...
          ld.object_coords = sq
            ...
            .reduce((a, b) => {
              return a.concat(
                ...[(b.x / width).toString(), (b.y / height).toString()]
              );
            }, []);
          break;
      }
        ...
        switch (eventType) {
          ...
          default:
            ...
            if (currCtnObj.inBounds) {
              if (
                // If shape's bounding box is within image bounds
                grB.x > 0 &&
                grB.y > 0 &&
                ...
                currCtnObj.lastSqPosArr = [];
                sq.forEach(s =>
                  currCtnObj.lastSqPosArr.push({ x: s.x, y: s.y })
                );
                // l(currCtnObj.lastSqPosArr)
              } else {
                inBounds = false;
  ...
  moveToTopLayer = () => {
    ...
    this.setState(
      ...
      () => {
        ...
        currLblEl.children[1].children[0].children[0].children[0].focus();
      }
    );
  };
    ...
    if (fromOptions) {
      this.http
        ...
        .catch(res => {
          // l(res)
          alert(
            "There is another label with this name. Please use a different name."
          );
        });
    } else {
      this.http
  ...
  render() {
    ...
    if (this.state.tempLblName.length > 1) {
      ...
      inputSize =
        this.state.tempLblName.length - Math.round(spaceCount / 2) - 1;
    ...
    let vicinityData =
        photo.place_vicinity && photo.place_vicinity.length
          ? photo.place_vicinity
          : null,
      contactData =
        photo.contact_data && photo.contact_data.length
          ? photo.contact_data
          : null;
      ...
      >
        <div>
          <div className="title row pb-0">
            {photos.length > 0 ? (
                    ...
                    <div className="col-lg-12">
                      ...
                      {vicinityData && (
                        <div className="photo-info">
                          <span title={vicinityData}>{vicinityData}</span>
                        </div>
                      )}
                      {contactData && (
                        <div className="photo-info">
                          <span title={contactData}>{contactData}</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="col-lg-12 pb-4">No Photos available.</div>
            )}
          </div>
          <div className="body">
            <div className="row">
                    ...
                    <div className="row pb-0">
                      <div className="col-1">
                        <div
                          className={
                            "obj-sel " +
                            (this.state.adding === "polygon" ? "active" : "")
                          }
                          onClick={e => this.startAdding(e, "polygon")}
                        >
                          <img src="assets/poly.svg" alt="" />
                          {this.state.adding === "polygon" && (
                            <div
                              className="ctn-add-action"
                              onClick={e => e.stopPropagation()}
                            >
                              <FontAwesomeIcon
                                onClick={e => this.doneAdding(e, "Polygon")}
                                icon={faCheck}
                                style={{ cursor: "pointer", color: "green" }}
                              />
                              <FontAwesomeIcon
                                onClick={e => this.resetForAdding(e)}
                                icon={faTimes}
                        ...
                        <div
                          className={
                            "obj-sel " +
                            (this.state.adding === "rectangle" ? "active" : "")
                          }
                          onClick={e => this.startAdding(e, "rectangle")}
                        >
                          <img src="assets/square.svg" alt="" />
                          {this.state.adding === "rectangle" && (
                            <div
                              className="ctn-add-action"
                              onClick={e => e.stopPropagation()}
                            >
                              <FontAwesomeIcon
                                onClick={e => this.doneAdding(e, "Rectangle")}
                                icon={faCheck}
                                style={{ cursor: "pointer", color: "green" }}
                              />
                              <FontAwesomeIcon
                        ...
                        <div
                          className={
                            "obj-sel " +
                            (this.state.adding === "circle" ? "active" : "")
                          }
                          onClick={e => this.startAdding(e, "circle")}
                        >
                          <img src="assets/circle.svg" alt="" />
                          {this.state.adding === "circle" && (
                            <div
                              className="ctn-add-action"
                              onClick={e => e.stopPropagation()}
                            >
                              <FontAwesomeIcon
                                onClick={e => this.doneAdding(e, "Circle")}
                                icon={faCheck}
                                style={{ cursor: "pointer", color: "green" }}
                              />
                              <FontAwesomeIcon
                      ...
                      <div className="col-11">
                        <div
                          ...
                          className={
                            "ctn-photo " +
                            (this.state.imgOrientation === "landscape"
                              ? "ctn-horizontal"
                              : "ctn-vertical")
                          }
                        >
                              ...
                              {lbl.form === "Rectangle" && (
                                <svg width="24" height="24" viewBox="0 0 24 24">
                                  <defs>
                                    <path
                                      id="b"
                                      d="M5 4a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1H5zm0-2h14a3 3 0 0 1 3 3v14a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3V5a3 3 0 0 1 3-3z"
                                    />
                                  </defs>
                                  <use
                                    fill={lbl.color}
                ...
                <div className="row">
                  <div className="col-lg-6">
                    <div className="custom-control custom-checkbox">
                      <input
                        checked={this.state.ml ? "checked" : ""}
                        onChange={this.mlChanged}
                        type="checkbox"
        ...
        <div>
          ...
          <div className="body">
            ...
            <div className="row">
              <div className="col-lg-12">
                <button onClick={this.doUpload} className="btn btn-accent">
                  Upload
                </button>
                <button
                  onClick={this.cancelUpload}
                  className="ml-3 btn btn-accent-outline"
                >
                  Cancel
                </button>
              </div>
